  ...
  const fetchTimesheetData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/extra-timesheets/daily?date=${dateStr}&departmentId=${realIds.join(",")}`,
      );
        ...
        {employees.length > 0 && (
          <div style={{ marginBottom: 6 }}>
            {timesheetStatus?.isSubmitted ? (
              ...
              <Alert
                message="Chưa có dữ liệu công tăng cường."
                type="info"
                showIcon
                style={{ fontSize: 11, padding: "4px 8px" }}
              />
...
export default function MobileExtraTimesheetDailyPage() {
  return (
    <Suspense
      fallback={
        <div
