...
export default function MobileTimesheetPage() {
  ...
  const fetchEmployeeForMonth = async (empId: number, newMonth: Dayjs) => {
    ...
    try {
      const month = newMonth.month() + 1,
        year = newMonth.year();
      let url = `/api/timesheets/monthly?month=${month}&year=${year}`;
      if (currentFilter.factoryId)
        url += `&factoryId=${currentFilter.factoryId}`;
      ...
      const data = await res.json();
      if (!data.error) {
        const found = (data as MonthlyEmployeeData[]).find(
          (e) => e.id === empId,
        );
        if (found) {
  ...
  const logsByDate = useMemo(() => {
    ...
    (selectedEmployee?.timesheets || []).forEach((t) =>
      map.set(t.date.slice(0, 10), t.attendanceCode),
    );
    return map;
  }, [selectedEmployee]);
    ...
    return (
      <div style={pageStyle}>
        ...
        >
          ...
          <div style={{ fontSize: 11, opacity: 0.8, flexShrink: 0 }}>
            {employees.length} NV
          </div>
          <Link href="/">
            <Button
            ...
            {[
              ...
              <div
                ...
                style={{
                  ...
                  gap: 8,
                  padding: "8px 10px",
                  background: item.value === 0 ? "#fafafa" : "#f0f7ff",
                  borderRadius: 8,
                  border: `1px solid ${item.value === 0 ? "#f0f0f0" : "#bae0ff"}`,
                  opacity: item.value === 0 ? 0.6 : 1,
                }}
              >
                ...
                <div>
                  ...
                  <div
                    style={{ fontSize: 15, fontWeight: 700, lineHeight: 1.2 }}
                  >
                    {item.value}
                  </div>
                </div>
