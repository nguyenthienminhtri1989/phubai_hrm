...
import { Button, Drawer, Badge, Spin, message, Tag } from "antd";
import {
  FilterOutlined,
  HomeOutlined,
  BarChartOutlined,
...
export default function MobileYearlyPage() {
  ...
  const filterApplied = !!(
    activeFilter &&
    activeFilter.realDepartmentIds &&
    activeFilter.realDepartmentIds.length > 0
  );
    ...
    if (reportType === "leave") {
      ...
      return (
        <div style={{ display: "flex", gap: 4, marginTop: 6 }}>
          ...
          >
            Đã nghỉ: {row.summary.col1}
          </span>
          <span
            style={{
              background: rem < 0 ? "#fff2f0" : "#f6ffed",
              border: `1px solid ${rem < 0 ? "#ffccc7" : "#b7eb8f"}`,
              borderRadius: 4,
              padding: "1px 6px",
              fontSize: 11,
              color: rem < 0 ? "#ff4d4f" : "#52c41a",
              fontWeight: 700,
            }}
          >
            Còn: {rem}
    ...
    >
      ...
      >
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <Button
            ...
            disabled={
              !currentFilter || currentFilter.realDepartmentIds.length === 0
            }
          >
            Xem báo cáo →
          </Button>
      ...
      <div style={{ padding: "8px 8px 0" }}>
        {loading ? (
          ...
          >
            <FilterOutlined
              style={{ fontSize: 32, marginBottom: 10, color: "#ccc" }}
            />
            <div style={{ fontWeight: 600, marginBottom: 6, fontSize: 13 }}>
              Chưa chọn bộ lọc
            </div>
            <div style={{ fontSize: 12 }}>
              Bấm nút <FilterOutlined /> góc trên để chọn năm và bộ phận
            </div>
          </div>
        ) : reportData.length === 0 ? (
          <div
              ...
              >
                ...
                >
                  {MONTHS.map((m, i) => (
                    ...
                    >
                      <div
                        style={{ fontSize: 9, color: "#aaa", lineHeight: 1.2 }}
                      >
                        {m}
                      </div>
                      <div style={{ lineHeight: 1.4 }}>
                        {renderCell(row.data[i], reportType)}
                      </div>
                    </div>
                  ))}
                </div>
