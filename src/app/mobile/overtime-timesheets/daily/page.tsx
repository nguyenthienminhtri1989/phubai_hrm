...
function MobileDailyTimesheetInner() {
  ...
  const fetchTimesheetData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/overtime-timesheets/daily?date=${dateStr}&departmentId=${realIds.join(",")}`,
      );
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch("/api/overtime-timesheets/daily", {
        method: "POST",
        ...
        {employees.length > 0 && (
          <div style={{ marginBottom: 6 }}>
            {timesheetStatus?.isSubmitted ? (
              ...
              <Alert
                message="Chưa có dữ liệu công thêm giờ."
                type="info"
                showIcon
                style={{ fontSize: 11, padding: "4px 8px" }}
              />
...
export default function MobileOvertimeTimesheetDailyPage() {
  return (
    <Suspense
      fallback={
        <div
