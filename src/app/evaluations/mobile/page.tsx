...
function MobileEvaluationInner() {
  ...
  const [changes, setChanges] = useState<
    Record<number, { grade: string; note: string }>
  >({});
  ...
  useEffect(() => {
    ...
    if (fParam) {
      const id = parseInt(fParam);
      if (!isNaN(id)) setSelectedFactoryId(id);
    }
    if (dParam)
      setMixedDeptValues(
        dParam
  ...
  const getGrade = (empId: number) =>
    changes[empId]?.grade !== undefined
      ? changes[empId].grade
      : data.find((d) => d.id === empId)?.grade || "";
  const getNote = (empId: number) =>
    changes[empId]?.note !== undefined
      ? changes[empId].note
      : data.find((d) => d.id === empId)?.note || "";

  const handleGradeChange = (empId: number, val: string) => {
    setChanges((prev) => ({
      ...prev,
      [empId]: { grade: val, note: prev[empId]?.note ?? getNote(empId) },
    }));
  };
  const handleNoteChange = (empId: number, val: string) => {
    setChanges((prev) => ({
      ...prev,
      [empId]: { grade: prev[empId]?.grade ?? getGrade(empId), note: val },
    }));
  };
  ...
  const filterLabel = useMemo(() => {
    ...
    const deptLabels = mixedDeptValues.map(
      (v) => mixedDeptOptions.find((o) => o.value === v)?.label || v,
    );
  ...
  return (
    ...
    >
        ...
        >
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontWeight: 700, fontSize: 15 }}>
              Xếp Loại Tháng {date.format("MM/YYYY")}
            </div>
            {filterLabel && (
              <div
                style={{
      ...
      <div style={{ padding: "8px 8px 0" }}>
        {data.length > 0 && (
          <Alert
            message={`Đã xếp loại: ${gradedCount}/${data.length} người${changesCount > 0 ? ` · ${changesCount} thay đổi chưa lưu` : ""}`}
            type={changesCount > 0 ? "warning" : "success"}
            showIcon
        ...
        {canEdit && data.length > 0 && (
          ...
          >
            <span style={{ fontSize: 11, color: "#666", flexShrink: 0 }}>
              <ThunderboltOutlined style={{ color: "#faad14" }} /> Tất cả:
            </span>
            {["A", "A-", "B", "B-", "C"].map((g) => (
              <Popconfirm
                key={g}
                title={`Xếp loại ${g} tất cả?`}
                onConfirm={() => handleQuickFill(g)}
              >
                <Button
                  size="small"
                  style={{
      ...
      {loading ? (
        ...
        >
          ...
          <div style={{ fontSize: 12 }}>
            Bấm nút <FilterOutlined /> góc trên để chọn tháng và bộ phận
          </div>
        </div>
      ) : (
        <div
            ...
            return (
              ...
              >
                ...
                <Select
                  ...
                  onChange={(val) => handleGradeChange(emp.id, val)}
                  ...
                  options={["A", "A-", "B", "B-", "C"].map((g) => ({
                    value: g,
                    label: (
                      <span style={{ color: GRADE_COLORS[g], fontWeight: 700 }}>
                        {g}
                      </span>
                    ),
                  }))}
