...
export default function MonthlyTimesheetPage() {
  ...
  const fetchMonthlyData = async (filter: FilterResult) => {
    ...
    try {
      ...
      let url = `/api/overtime-timesheets/monthly?month=${month}&year=${year}`;
  ...
  const handleLoadAll = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/overtime-timesheets/monthly?month=${month}&year=${year}&view=all`);
      const data = await res.json();
  ...
  const handleExportExcel = async () => {
    ...
    const worksheet = workbook.addWorksheet("CongThemGio");
    ...
    row2.value = `BẢNG CÔNG THÊM GIỜ THÁNG ${currentFilter.date.format("MM/YYYY")}`;
    ...
    saveAs(blob, `BangCongThemGio_${currentFilter.date.format("MM-YYYY")}.xlsx`);
    setLoading(false);
  };
  ...
  return (
    <AdminLayout>
      <div style={{ marginBottom: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Title level={3} style={{ margin: 0 }}>Tổng hợp công thêm giờ tháng</Title>
