...
export default function MonthlyTimesheetPage() {
  ...
  const fetchMonthlyData = async (filter: FilterResult) => {
    ...
    try {
      ...
      let url = `/api/extra-timesheets/monthly?month=${month}&year=${year}`;
  ...
  const handleLoadAll = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/extra-timesheets/monthly?month=${month}&year=${year}&view=all`);
      const data = await res.json();
  ...
  const handleExportExcel = async () => {
    ...
    const worksheet = workbook.addWorksheet("CongTangCuong");
    ...
    row2.value = `BẢNG CÔNG TĂNG CƯỜNG THÁNG ${currentFilter.date.format("MM/YYYY")}`;
    ...
    saveAs(blob, `BangCongTangCuong_${currentFilter.date.format("MM-YYYY")}.xlsx`);
    setLoading(false);
  };
  ...
  return (
    <AdminLayout>
      <div style={{ marginBottom: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Title level={3} style={{ margin: 0 }}>Tổng hợp công tăng cường tháng</Title>
